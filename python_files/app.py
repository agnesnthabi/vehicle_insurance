...
def main():
    ...
    img_base64 = get_base64_image("../images/final.png")
    ...
    <div style="width: 100vw; 
                ...
                padding: 4rem 4rem; 
                text-align: center;">
        <img src="data:image/png;base64,{img_base64}" width="300" style="margin-bottom: 1rem;" />
        <p style="font-size: 1.8em; color: white; font-weight: bold;">
...
""", unsafe_allow_html=True)
    ...
    with tab1:
        st.subheader("")
        st.markdown('<p style="text-align: center; font-size: 2em; color: #4c416bff; font-weight: bold; ">Individual Customer Prediction</p>', unsafe_allow_html=True)
        ...
        centered_button = st.columns([1, 2, 1])
    ...
    with tab3:
        ...
        if hasattr(model, 'feature_importances_'):
            ...
            if feature_names is not None and len(feature_names) == len(model.feature_importances_):
                ...
                chart = alt.Chart(importance_df.reset_index()).mark_bar(color='white').encode(
                    x='Feature',
                    y='Importance'
                    ).properties(
                        width=1250,
        ...
        try:
            ...
            card_style = """
                    ...
                    ">
                        ...
                        <p style="font-size: 1.8rem; font-weight: bold;">{value}</p>
                    </div>
            """
            with col1:
                st.markdown(card_style.format(title="F1 Score", value=f"{f1:.3f}"), unsafe_allow_html=True)

            with col2:
                st.markdown(card_style.format(title="Precision", value=f"{precision:.3f}"), unsafe_allow_html=True)

            with col3:
                st.markdown(card_style.format(title="Recall", value=f"{recall:.3f}"), unsafe_allow_html=True)
